...
<script>
      document.querySelector('.duplicateBtn').addEventListener('click', (e) =>
    {
    const dupCard = document.querySelector('.wrap').cloneNode(true);
      document.body.appendChild(dupCard);
    });
    document.querySelector('.changeBckgrnd').addEventListener('click', (e) => 
    {
      const bckColor = document.querySelector('.wrap');
      bckColor.style.backgroundColor = 'black';
    });
    document.querySelector('.ChangeTitle').addEventListener('click',(e) =>
    {
      document.querySelector('h3').innerText = 'Marvel Cinematic Universe';
    });
    document.querySelector('.Delcrd').addEventListener('click', (e) =>
    {
      const allCrds = document.querySelectorAll(".wrap");
      if(allCrds.length > 1)
            {
              const finalCard = allCrds[allCrds.length-1];
              document.body.removeChild(finalCard);
      }
    });
</script>
    `;
